...
class FinrespSupabaseUser extends BaseAuthUser {
  FinrespSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> finrespSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FinrespSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
